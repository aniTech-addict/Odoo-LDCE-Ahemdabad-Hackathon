CREATE TABLE IF NOT EXISTS cities (
    ...
    CONSTRAINT chk_city_popularity CHECK (
        popularity IS NULL OR (popularity >= 0 AND popularity <= 100)
    )
);
...
CREATE INDEX IF NOT EXISTS idx_itinerary_trip_day
    ON itinerary_items(trip_id, day_date);
CREATE INDEX IF NOT EXISTS idx_itinerary_activity_id
